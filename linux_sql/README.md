...
# Introduction
The Jarvis Linux Cluster Monitoring project is designed to provide the user with the tools necessary to easily record the hardware and real-time usage data of 10 nodes/servers. The project collects the data from all 10 nodes/servers and inserts it into an RDMS hosted on one of the machines. This project was designed with the intention of making it quick and simple to set up, as well as being easily scalable for future expansions. To achieve this, bash scripts were used to collect data and to preform first-time setup, docker was used to run PostgresSQL, Git was used for version control, and crontab was used for automation.
...
2. Create tables using ddl.sql (done on psql host machine)
    ...
    ```
3.  Insert hardware specs data into the DB using host_info.sh (done on each machine once | [psql_host] will be localhost of done on the host machine)
    ```
    ./linux_sql/scripts/host_info.sh [psql_host] 5432 host_agent [db_username] [db_password]
    ```
4. Insert hardware usage data into the DB using host_usage.sh (this inserts the usage data once, to automate, look at the next step | [psql_host] will be localhost of done on the host machine)
    ```
    ./linux_sql/scripts/host_usage.sh [psql_host] 5432 host_agent [db_username] [db_password]
    ```
5. Crontab setup (done on each machine | runs every minute | inserts output to a log file local to each machine)
    ```
    crontab -e
...
**[db_username]** - set a username used to log in to postgres  
**[db_password]** - sets a password used to log in to postgres
```
./linux_sql/scripts/psql_docker.sh create [db_username] [db_password] 
```
...
**[psql_host]** - ip where the psql is located (localhost if on the host machine)  
**[psql_port]** - port opened for the psql database (5432 default)  
**[db_name]** - name of the database (host_agent default)  
**[db_username]** - username set with [psql_docker.sh create](#psql_dockersh)  
**[db_password]** - password set with [psql_docker.sh create](#psql_dockersh)
...
# Testing
All testing was completed on a single machine running rocky 9 on Google Cloud. It was assumed that all firewalls, networking, and connections were set up properly.
First, I ran each script the way it was meant to be run and verified I got the intended result in the database. Then I ran each script in an improper way to make sure I got the proper error messages and error codes.
## Some sample test
|script | test| result |
| :---- | :---- | :---- |
...
- setup alerts if a machine's usage stat passes a certain value there may be something wrong with that machine